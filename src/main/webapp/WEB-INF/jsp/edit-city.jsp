                            ...
                            <form method="POST" action="${pageContext.request.contextPath}/saveCity" autocomplete="off">
                                <div class="form-group col-md-12">
                                    ...
                                    <c:forEach var="city" items="${activeStateList}">
                                     <option value="${city.sk_state_id }" <c:if test = "${city.sk_state_id == cityDetails.state_id }">selected </c:if>>${city.state_name}</option>
                                     </c:forEach>
                                     </select>                           
                                 </div>
                                 <div class="form-group col-md-12">
                                    <label for="state_name">City Name<span class="text-danger">*</span></label>
                                    <input type="text" required   class="form-control" onkeyup="checkCityExistOrNot(this.value)" id="city_name" name="city_name" value="${cityDetails.city_name}" placeholder="Enter City Name" data-parsley-pattern="^[a-zA-Z ]+$"  data-parsley-maxlength="100">
                                    <input type="hidden" class="form-control"  id="city_name" name="sk_city_id" value="${cityDetails.sk_city_id}" placeholder="Enter City Name" data-parsley-pattern="^[a-zA-Z ]+$"  data-parsley-maxlength="100">
                                    <span id="errormsg" style="color:red"></span>
                                 <input type="text"   id="testbox" style="display: none">
                                </div>
                                 <div class="form-group col-md-12">
                                    <label for="state_name">Tier<span class="text-danger">*</span></label>
                                    <input type="text" required   class="form-control" id="tier" name="tier" placeholder="Enter Tier" value="${cityDetails.tier}"   data-parsley-maxlength="100">
                                </div>
                                 <div class="form-group col-md-12">
                                    <label for="state_name">Metro<span class="text-danger">*</span></label>
                                    <input type="text" required   class="form-control"  id="metro" name="metro" placeholder="Enter Metro" value="${cityDetails.metro}"   data-parsley-maxlength="100">
                                </div>
                                <div class="form-group text-right m-b-0">
                                    <button class="btn btn-gradient waves-effect waves-light btnsubmit" type="submit">
                                        Update
                                    </button>
                                    <a href="${pageContext.request.contextPath}/addCity"><button type="submit" class="btn btn-light waves-effect m-l-5 resetbtn" id="reset" >
                                        Cancel
                                    </button></a>
    ...
    <!-- state existeor not -->
      ...
      function exist(name){
    	  var state_id=$("#state_id").val();
      	 $.ajax({
               url: "${pageContext.request.contextPath}/getCityExistance",
               type: "GET", 
